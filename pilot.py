...
def counterfact_loss(E, off, W, H, M, t, task, B, old_actions, gains, gainsK, old_pivots, state):
    y, cost = state, 0
    for h in range(H):
        u_extra = np.tensordot(E, W[h:h+M], axes = ([0, 2], [0, 1])) + off
        new_action = old_actions[t+h]+gains[t+h]+gainsK[t+h]@(y-old_pivots[t+h]) + B[t+h] @ u_extra
...
def moo(task, old_actions, gains, gainsK, old_pivots, lr=1e-6, is_real_dynamics=True, is_real_derivatives=False):
    H, M = 10, 10
    states, new_actions, cost_params, d_params, cost, B = [task.initial_state], [], [], [], 0, []
    E, W, off = np.zeros((H, task.state_size, task.state_size)), np.zeros((H+M, task.state_size)), np.zeros(task.state_size)
    for j in range(task.h):
        u_extra = np.tensordot(E, W[-M:], axes = ([0, 2], [0, 1])) + off

        new_action = old_actions[j]+gains[j]+gainsK[j]@(states[j]-old_pivots[j])
        B.append(onp.linalg.pinv(task.dynamics_grad(states[j], new_action, j, is_real_derivatives)[1]))
        new_action += B[j] @ u_extra
        new_actions.append(new_action)  
        states.append(task.step(states[j],new_action,j, is_real_dynamics)+u_extra)
        ...
        if j >= H:
            delta_E, delta_off = grad_loss(E, off, W, H, M, j-H, task, B, old_actions, gains, gainsK, old_pivots, states[j-H])
            E -= lr * delta_E
            off -= lr * delta_off
            if j % 20 == 0:
                print(off, np.average(W, axis = 0))
